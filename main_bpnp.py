...
def build_bpnp_correspondences_torch(flow_up, lidar_input, calib, max_points=4096):
    ...
    base_uv = torch.stack([xs, ys], dim=-1)  # [H, W, 2]

    correspondences = []

    for n in range(B):
        depth = lidar_input[n, 0].to(device=device, dtype=dtype)  # [H, W]
        ...
        uv_src = base_uv[valid]  # [N, 2]

        flow_valid = flow_up[n].permute(1, 2, 0)[valid]  # [N, 2]
        pts2d = uv_src + flow_valid  # [N, 2]
        ...
        pts2d = pts2d[in_img]
        uv_src = uv_src[in_img]
        depth_valid = depth[valid][in_img]
        ...
        if pts2d.shape[0] > max_points:
            ids = torch.randperm(pts2d.shape[0], device=device)[:max_points]
            pts2d = pts2d[ids]
            uv_src = uv_src[ids]
            depth_valid = depth_valid[ids]

        fx, fy, cx, cy = calib[n].to(device=device, dtype=dtype)
        ...
        pts3d = torch.stack([x, y, z], dim=-1)  # [N, 3]
        ...
        correspondences.append((pts2d.unsqueeze(0), pts3d, K))
...
def train(args, epoch, TrainImgLoader, model, optimizer, scheduler, scaler, logger, device):
    ...
    for i_batch, sample in enumerate(TrainImgLoader):
        ...
        rgb_input, lidar_input, flow_gt = data_generate.push(
            rgb, pc, T_err, R_err, device
        )

        depth_img_input = []
        for i in range(lidar_input.shape[0]):
            depth_img = lidar_input[i, 0, :, :].cpu().numpy() * 100.0
            depth_img_dilate = sparse_to_dense(depth_img.astype(np.float32))
            depth_img_input.append(depth_img_dilate / 100.0)
        ...
        loss = loss + norm_loss * 100.0

        flow_up = flow_preds[-1]

        if epoch > BPnP_EPOCH:
            correspondences = build_bpnp_correspondences_torch(
                flow_up=flow_up,
            ...
            loss_poses = flow_up.sum() * 0.0
            valid_pose_num = 0

            for n, corr in enumerate(correspondences):
                if corr is None:
                    continue
                ...
                P_gt = P_gt.unsqueeze(0).to(
                    device=flow_up.device,
                    dtype=flow_up.dtype
                )

                pts2d_gt = batch_project(P_gt, pts3d, K).detach()

                loss_poses = loss_poses + ((pts2d_pro - pts2d_gt) ** 2).mean()
                valid_pose_num += 1

            if valid_pose_num > 0:
                loss_pose = loss_poses / valid_pose_num
            else:
                loss_pose = flow_up.sum() * 0.0
        else:
            loss_pose = flow_up.sum() * 0.0

        loss = loss + loss_pose * 10.0
...
def test(args, TestImgLoader, model, bpnp, device, cal_pose=False):
    ...
    for i_batch, sample in enumerate(TestImgLoader):
        ...
        data_generate = Data_preprocess(calib, occlusion_threshold, occlusion_kernel)
        rgb_input, lidar_input, flow_gt = data_generate.push(
            rgb, pc, T_err, R_err, device, split='test'
        )
        ...
        if args.render:
            if not os.path.exists("./visulization"):
                os.mkdir("./visulization")
                os.mkdir("./visulization/flow")
                os.mkdir("./visulization/original_overlay")
                os.mkdir("./visulization/warp_overlay")

            flow_image = flow_to_image(
                flow_up.permute(0, 2, 3, 1).cpu().detach().numpy()[0]
            )
            ...
            output = visibility.image_warp_index(
                lidar_input.to(device),
                flow_up.int().to(device),
                pred_depth_img,
                output,
                lidar_input.shape[3],
                lidar_input.shape[2]
            ...
            original_overlay = overlay_imgs(
                rgb_input[0, :, :, :],
                lidar_input[0, 0, :, :]
            )
            cv2.imwrite(
                f'./visulization/original_overlay/{i_batch:06d}.png',
                original_overlay
            )

            warp_overlay = overlay_imgs(
                rgb_input[0, :, :, :],
                pred_depth_img[0, 0, :, :]
            )
            cv2.imwrite(
                f'./visulization/warp_overlay/{i_batch:06d}.png',
                warp_overlay
            )
        ...
        if not cal_pose:
            ...
            print(
                f"{i_batch:05d}: "
                f"{np.mean(err_t_list):.5f} "
                f"{np.mean(err_r_list):.5f} "
                f"{np.median(err_t_list):.5f} "
                f"{np.median(err_r_list):.5f} "
                f"{len(outliers)} "
                f"{Time / (i_batch + 1):.5f}"
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        metavar='N',
        help='Evaluate every evaluate_interval epochs'
    )
    parser.add_argument(
        '-e',
        '--evaluate',
    ...
    dataset_test = DatasetVisibilityKittiSingle(
        args.data_path,
        max_r=args.max_r,
        max_t=args.max_t,
        split='test',
    ...
    if args.evaluate:
        with torch.no_grad():
            err_t_list, err_r_list, outliers, Time = test(
                args,
                TestImgLoader,
                model,
                bpnp,
            ...
            print(
                f"Mean trans error {np.mean(err_t_list):.5f}  "
                f"Mean rotation error {np.mean(err_r_list):.5f}"
            )
            print(
                f"Median trans error {np.median(err_t_list):.5f}  "
                f"Median rotation error {np.median(err_r_list):.5f}"
            )
            print(
                f"Outliers number {len(outliers)}/{len(TestImgLoader)}  "
                f"Mean {Time / len(TestImgLoader):.5f} per frame"
            )

        sys.exit()

    dataset_train = DatasetVisibilityKittiSingle(
        args.data_path,
        max_r=args.max_r,
        max_t=args.max_t,
        split='train',
