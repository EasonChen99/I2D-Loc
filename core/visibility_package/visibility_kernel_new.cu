...
std::vector<at::Tensor> visibility_filter_cuda2(at::Tensor input_depth, at::Tensor intrinsic, at::Tensor in_index, at::Tensor output, at::Tensor out_index, unsigned int width,unsigned int height, float threshold, unsigned int radius) {
    ...
    AT_DISPATCH_FLOATING_TYPES(input_depth.type(), "visibility_filter2", ([&] {
        visibility_kernel2<scalar_t><<<blocks, threads>>>(input_depth.data<scalar_t>(), intrinsic.data<scalar_t>(), in_index.data<scalar_t>(),
                                                          output.data<scalar_t>(),out_index.data<scalar_t>(), width, height, threshold, radius);}));
    //visibility_kernel<float><<<blocks, threads>>>(input.data<float>(), output.data<float>(), width, height, threshold);
...
at::Tensor image_warp_index_cuda(at::Tensor input, at::Tensor flow, at::Tensor warp_depth, at::Tensor output, unsigned int width, unsigned int height)
{
    dim3 threads(32, 32);
    dim3 blocks(height/32+1, width/32+1);
    AT_DISPATCH_FLOATING_TYPES(input.type(), "image_warp_index",([&] {
    image_warp_index_kernal<<<blocks, threads>>>(input.data<scalar_t>(), flow.data<int>(), warp_depth.data<scalar_t>(), output.data<scalar_t>(), width, height);}));
    return output;
}
...
at::Tensor image_warp_cuda(at::Tensor input, at::Tensor flow, at::Tensor output, unsigned int width, unsigned int height)
{
    dim3 threads(32, 32);
    dim3 blocks(height/32+1, width/32+1);
    AT_DISPATCH_FLOATING_TYPES(input.type(), "image_warp",([&] {
    image_warp_kernal<<<blocks, threads>>>(input.data<scalar_t>(), flow.data<int>(),output.data<scalar_t>(), width, height);}));
    return output;
}
