...
from utils_point import rotate_forward, rotate_back, invert_pose
    ...
    def __init__(self, dataset_dir, transform=None, augmentation=False, maps_folder='local_maps_0.1',
        ...
        for dir in ['00', '03', '05', '06', '07', '08', '09']:
            ...
            for index, row in df_locations.iterrows():
                if not os.path.exists(os.path.join(dataset_dir, dir, maps_folder, f"{int(row['timestamp']):06d}"+'.h5')):
                    continue
                if not os.path.exists(os.path.join(dataset_dir, dir, 'image_2', f"{int(row['timestamp']):06d}"+'.png')):
                    continue
                if dir == test_sequence and split.startswith('test'):
                    self.all_files.append(os.path.join(dir, f"{int(row['timestamp']):06d}"))
