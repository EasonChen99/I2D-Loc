...
class Flow2PoseNet(nn.Module):
    def __init__(self, args):
        super(Flow2PoseNet, self).__init__()

        self.flowNet = RAFT(args)
        self.flowPoseNet = VOFlowRes()

    def forward(self, dense_depth, RGB_img, sparse_depth, intrinsicLayer, test_mode=False):
        if not test_mode:
            flow_predictions = self.flowNet(dense_depth, RGB_img, lidar_mask=sparse_depth, iters=args.iters)
            flow_up = flow_predictions[-1]
        else:
            _, flow_up = self.flowNet(dense_depth, RGB_img, lidar_mask=sparse_depth, iters=24, test_mode=True)

        mask = sparse_depth[:, 0, :, :] > 0
        flow_up[:, 0, :, :] = flow_up[:, 0, :, :] * mask
        flow_up[:, 1, :, :] = flow_up[:, 1, :, :] * mask

        flow_input = torch.cat((flow_up, intrinsicLayer), dim=1)
        pose = self.flowPoseNet(flow_input)

        if not test_mode:
            return flow_predictions, pose
        else:
            return flow_up, pose